...
namespace TotallyMoney.TrustPilotLinks.Web.Logic
{
    public class Generator
    {
        ...
        public static string GetBase64(string email, Encoding enc)
        {
        ...
        public static string GetUrlEncodedName(string name)
        {
            var encodedName = HttpUtility.UrlPathEncode(name);
            return encodedName;
        }

        public static string CalculateHash(string key, string email, string orderId)
        {
            var stringInput = key + email + orderId;
            var asciiEnc = Encoding.ASCII;
      
            var byteInput = asciiEnc.GetBytes(stringInput);
        ...
        public static string GetUniqueLink(string domain, string orderId, string base64Email, string encodedName, string hash)
        {
